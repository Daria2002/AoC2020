...
class AdvancedMessenger
{
    public:
        ...
        AdvancedMessenger(std::string file_name)
        {
            std::ifstream file(file_name);
            std::string line;
            ReadMode read_mode = ReadMode::rules;
        ...
        {
            int rule_number = std::stoi(line.substr(0, line.find(":")));
            if(rule_number == 0 || rule_number == 8 || rule_number == 11)
            {
                // skip special cases
                return;
            }
...
{
    std::cout << "======\nPart 2\n======\n";
    AdvancedMessenger messenger(file_name); // builds rules and stores messages to check
